...
def plot_spectrum(yvals):
    ...
    ax.set_title('Normalised eigenspectrum for {} largest eigenvalues\n Model trained on label = 7, epoch = 1000'.format(num_eigs))

    # x axis ticks
    ax.xaxis.set_major_locator(MultipleLocator(1))

    # save plot
    plt.savefig(outPath + 'L7avgindep-top{}.png'.format(num_eigs))
    print('the plot has been saved as L7avgindep-top{}.png'.format(num_eigs))
...
if __name__ == "__main__":
    ...
    for b in range(13): # beta runs from b = 0 to 12
        ...
        if ytopavg.size == 0:
            ytopavg = np.divide(ytopnorm, 12)
        else:
            ytopavg = np.vstack((ytopavg, np.divide(ytopnorm, 12)))
