...
def plot_spectrum(yvals):
    ...
    # set x and y range
    ax.set_ylim([ymin - buffer, 1 + buffer])
...
if __name__ == "__main__":
    ...
    for b in range(13): # beta runs from b = 0 to 12
        ...
        for m in range(2,10): # models run from l71 to l79
            model = 'l7{}indep'.format(m)
        ...
        if ytopavg.size == 0:
            ytopavg = np.divide(ytopnorm, 8)
        else:
            ytopavg = np.vstack((ytopavg, np.divide(ytopnorm, 8)))
