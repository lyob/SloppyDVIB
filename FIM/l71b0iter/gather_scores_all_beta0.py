...
def calc_original():    
    ...
    with sess0 as sess:
        modelpath = basepath+'/model/iterated-beta{}-15000'.format(beta)
        ...
        pickle.dump( out, open( basepath+'/scores/orig_network-b{}.p'.format(beta), "wb" ) )
...
def save_to_json(filename, object):
    with open(basepath+'/scores/'+filename, 'w') as f:
        ujson.dump(object, f)
    print("The data has been jsonified and saved as {}.".format(filename))

def load_from_json(filename):
    with open(basepath+'/scores/'+filename, 'r') as f:
        object = ujson.load(f)
    return object
