...
├───<>-data-processing.py
└───<>-data-collection.py
where <> is the filename (equal to the variable "filename")
...
# import dataset
# from h1024b32 import small_mnist
import small_mnist
...
for beta_num in range(0,13): # range(0, 13) results in beta = 1e-0 to 1e-12
    ...
    def evaluate():
        if dataset == "test":
            ...
            print("dataset value should be either test or train")
        return IZY, IZX, acc, avg_acc, 1-acc, 1-avg_acc


    ## Training occurs here
    # csvname = "./new_networks/"+filename+"/csv-output/full/full-"+filename+"-beta"+str(beta_num)+'-'+dataset+'.csv'
    # dataname ='./new_networks/'+filename+'/DATA/'+dataset+'/'+filename+'-beta'+str(beta_num)+'-'+dataset

    csvname = "./csv-output/full/full-"+filename+"-beta"+str(beta_num)+'-'+dataset+'.csv'
    dataname = './DATA/'+dataset+'/'+filename+'-beta'+str(beta_num)+'-'+dataset
