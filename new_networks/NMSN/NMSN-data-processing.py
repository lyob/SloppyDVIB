...
batch_size = 100
steps_per_batch = int(mnist_data.train.num_examples / batch_size) # keep as train
...
def evaluate(): # change to test or train
...
with open(output_file, "a") as b:
    ...
    for beta_num in range(13):
        ...
        modelPath = './'+filename+'/DATA/'+dataset+'/'+filename+'-beta'+str(beta_num)+'-'+dataset+'-120000' # trained model (graph/meta, index, data/weights)
